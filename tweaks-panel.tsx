...
// ── useTweaks ───────────────────────────────────────────────────────────────
const TWEAKS_LS_KEY = 'ledger_tweaks';

type TweakValues = Record<string, any>;
type SetTweak = (keyOrEdits: string | Record<string, any>, val?: any) => void;

function useTweaks(defaults: TweakValues): [TweakValues, SetTweak] {
  const [values, setValues] = React.useState<TweakValues>(() => {
    try {
      const stored = localStorage.getItem(TWEAKS_LS_KEY);
      if (stored) return { ...defaults, ...JSON.parse(stored) };
    } catch (_) {}
    return defaults;
  });
  const setTweak: SetTweak = React.useCallback((keyOrEdits: string | Record<string, any>, val?: any) => {
    const edits = typeof keyOrEdits === 'object' && keyOrEdits !== null
      ? keyOrEdits : { [keyOrEdits as string]: val };
    setValues((prev) => {
      const next = { ...prev, ...edits };
      try { localStorage.setItem(TWEAKS_LS_KEY, JSON.stringify(next)); } catch (_) {}
      return next;
    });
